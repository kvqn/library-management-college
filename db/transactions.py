...
class Transaction:
    def __init__(self, transaction_id : int, student_id : int, book_id : str, issue_datetime : datetime, return_datetime : datetime):
    ...
    def FetchFromDatabase(transaction_id : int):
        db.cur.execute(f"SELECT * FROM TRANSACTIONS WHERE transaction_id = {transaction_id}")
...
def BorrowBook(book_id : int, student_id : int):
    ...
    db.cur.execute(f"insert into TRANSACTIONS(book_id, student_id, issue_datetime) values ('{book_id}', {student_id}, '{datetime.now()}')")
...
def ReturnBook(book_id : int, student_id : int):
    ...
    db.cur.execute(f"update TRANSACTIONS set return_datetime = '{datetime.now()}' where book_id = '{book_id}' and student_id = {student_id} and return_datetime is null order by issue_datetime asc limit 1")
    books.increaseAvailableCount(book_id)

def checkIfAlreadyBorrowed(book_id : int, student_id : int):
    db.cur.execute(f"select count(*) from TRANSACTIONS where book_id = '{book_id}' and student_id = {student_id} and return_datetime is null")
