...
CART_MENU = interactables.AR_Menu(utils.Point(test.SCREEN_WIDTH-300,100), scale=0.004)
CART_MENU.items = []
...
def func_confirm():
    ...
    test.set_state(states.BORROW_INVOICE, books = CART_MENU.items.copy(), student = STUDENT)
    CART_MENU.items.clear()
    
BUTTON_CONFIRM = interactables.AR_Button.Create(test.SCREEN_WIDTH - 350, test.SCREEN_HEIGHT - 80, "Confirm", 0.03, utils.Color.GREEN, action=utils.Action(func_confirm), detection_thickness = 0.02)
...
BUTTON_CANCEL = interactables.AR_Button.Create(test.SCREEN_WIDTH - 150, test.SCREEN_HEIGHT - 80, "Cancel", 0.03, utils.Color.RED, action=utils.Action(func_cancel), detection_thickness = 0.02)
...
def cart_add(book_id : int):
    ...
    if db.books.checkIfCanBeBorrowed(book_id):
        if db.transactions.checkIfAlreadyBorrowed(book_id, STUDENT.id):
            TEXT_ERROR = interactables.Text(utils.Point(20,test.SCREEN_HEIGHT-20), 0.02, f"Book {book_id} is already borrowed", utils.Color.ORANGE, 2)
        ...
        TEXT_ERROR = interactables.Text(utils.Point(20,test.SCREEN_HEIGHT-20), 0.02, f"Book {book_id} is not available", utils.Color.RED, 2)
        time_text_error_start = time.time()
        ...
        for item in CART_MENU.items:
            if item.id == code:
                if code not in codes_on_screen:
                    TEXT_ERROR = interactables.Text(utils.Point(20,test.SCREEN_HEIGHT-20), 0.02, f"Book {code} is already in cart", utils.Color.ORANGE, 2)
                    time_text_error_start = time.time()
                break
        else:
            cart_add(code)
